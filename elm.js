...
Elm.Home.Feature.make = function (_elm) {
   ...
   var _op = {};
   var view = F2(function (address,model) {    return A2($Html.h1,_U.list([]),_U.list([$Html.text("Home Page")]));});
   var update = F2(function (action,model) {    return {ctor: "_Tuple2",_0: model,_1: $Effects.none};});
   var initialModel = {ctor: "_Tuple2",_0: {},_1: $Effects.none};
   var createHomeFeature = function (config) {    return $StartApp.start({init: initialModel,update: update,view: view,inputs: config.inputs});};
   var Model = {};
...
Elm.ComposeRouteMain.make = function (_elm) {
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var composeMainView = function (html) {
      return A2($Html.div,
      _U.list([]),
      _U.list([A2($Html.ul,_U.list([]),_U.list([A2($Html.li,_U.list([]),_U.list([$Html.text("Main")]))]))
              ,A2($Html.hr,_U.list([]),_U.list([]))
              ,A2($Html.div,_U.list([]),_U.list([html]))]));
   };
   var homeFeature = $Home$Feature.createHomeFeature({inputs: _U.list([]),outputs: {}});
   var html = A2($Signal.map,composeMainView,homeFeature.html);

...
Elm.Common.Model.make = function (_elm) {
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var initialModel = {message: "About Anything"};
   var Model = function (a) {    return {message: a};};
   return _elm.Common.Model.values = {_op: _op,Model: Model,initialModel: initialModel};
};
Elm.About = Elm.About || {};
Elm.About.Feature = Elm.About.Feature || {};
Elm.About.Feature.make = function (_elm) {
   "use strict";
   _elm.About = _elm.About || {};
   _elm.About.Feature = _elm.About.Feature || {};
   if (_elm.About.Feature.values) return _elm.About.Feature.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   ...
   var view = F2(function (address,model) {
      return A2($Html.div,
      _U.list([]),
      _U.list([A2($Html.h1,_U.list([]),_U.list([$Html.text("About Page")]))
              ,A2($Html.h2,_U.list([]),_U.list([$Html.text(A2($Basics._op["++"],"Message: ",model.message))]))]));
   });
   var createAboutFeature = function (config) {
      return $StartApp.start({init: {ctor: "_Tuple2",_0: $Common$Model.initialModel,_1: $Effects.none}
                             ,update: $Debug.watch("updating")
                             ,view: view
                             ,inputs: _U.list([])});
   };
   var update = F2(function (action,model) {    return A2($Debug.log,"action",{ctor: "_Tuple2",_0: model,_1: $Effects.none});});
   var Config = F2(function (a,b) {    return {inputs: a,outputs: b};});
   var NoOp = {ctor: "NoOp"};
   var Update = function (a) {    return {ctor: "Update",_0: a};};
   return _elm.About.Feature.values = {_op: _op,Update: Update,NoOp: NoOp,Config: Config,update: update,view: view,createAboutFeature: createAboutFeature};
};
...
Elm.Library.Util.make = function (_elm) {
   ...
   var broadcast = F3(function (outputs,data,action) {
      var check = A2($Debug.watch,"outputs",outputs);
   ...
   return _elm.Library.Util.values = {_op: _op,broadcast: broadcast};
};
Elm.Home = Elm.Home || {};
Elm.Home.Feature = Elm.Home.Feature || {};
Elm.Home.Feature.make = function (_elm) {
   "use strict";
   _elm.Home = _elm.Home || {};
   _elm.Home.Feature = _elm.Home.Feature || {};
   if (_elm.Home.Feature.values) return _elm.Home.Feature.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Common$Model = Elm.Common.Model.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   ...
   $StartApp = Elm.StartApp.make(_elm);
   var _op = {};
   var intoMessage = F2(function (model,message) {    return _U.update(model,{message: message});});
   var Config = F2(function (a,b) {    return {inputs: a,outputs: b};});
   var NoOp = {ctor: "NoOp"};
   var update = F3(function (services,action,model) {
      ...
      if (_p0.ctor === "Update") {
            var _p1 = _p0._0;
            return {ctor: "_Tuple2",_0: _p1,_1: A2(services.signalUpdate,_p1,NoOp)};
         } else {
   ...
   var view = F2(function (address,model) {
      return A2($Html.div,
      _U.list([]),
      _U.list([A2($Html.h1,_U.list([]),_U.list([$Html.text("Home Page")]))
              ,A2($Html.label,_U.list([$Html$Attributes.$for("message")]),_U.list([$Html.text("Message: ")]))
              ,A2($Html.input,
              _U.list([$Html$Attributes.name("message")
                      ,$Html$Attributes.value(model.message)
                      ,A3($Html$Events.on,
                      "change",
                      $Html$Events.targetValue,
                      function (msg) {
                         return A2($Signal.message,address,Update(A2(intoMessage,model,msg)));
                      })]),
              _U.list([]))
              ,A2($Html.span,_U.list([]),_U.list([A2($Html.em,_U.list([]),_U.list([$Html.text(model.message)]))]))]));
   });
   var createHomeFeature = function (config) {
      return $StartApp.start({init: {ctor: "_Tuple2",_0: $Common$Model.initialModel,_1: $Effects.none}
                             ,update: update({signalUpdate: $Library$Util.broadcast(config.outputs.onUpdatedMessage)})
                             ,view: view
                             ,inputs: config.inputs});
   };
   return _elm.Home.Feature.values = {_op: _op
                                     ,Update: Update
                                     ,NoOp: NoOp
                                     ,Config: Config
                                     ,intoMessage: intoMessage
                                     ,update: update
                                     ,view: view
                                     ,createHomeFeature: createHomeFeature};
};
Elm.ComposeRouteMain = Elm.ComposeRouteMain || {};
Elm.ComposeRouteMain.make = function (_elm) {
   "use strict";
   _elm.ComposeRouteMain = _elm.ComposeRouteMain || {};
   if (_elm.ComposeRouteMain.values) return _elm.ComposeRouteMain.values;
   var _U = Elm.Native.Utils.make(_elm),
   $About$Feature = Elm.About.Feature.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Common$Model = Elm.Common.Model.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Home$Feature = Elm.Home.Feature.make(_elm),
   $Html = Elm.Html.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var composeMainView = F2(function (homeHtml,aboutHtml) {
      return A2($Html.div,
      _U.list([]),
      _U.list([A2($Html.ul,_U.list([]),_U.list([A2($Html.li,_U.list([]),_U.list([$Html.text("Main")]))]))
              ,A2($Html.hr,_U.list([]),_U.list([]))
              ,A2($Html.div,_U.list([]),_U.list([homeHtml]))
              ,A2($Html.div,_U.list([]),_U.list([aboutHtml]))]));
   });
   var aboutFeatureMailbox = $Signal.mailbox($About$Feature.Update($Common$Model.initialModel));
   var homeFeature = $Home$Feature.createHomeFeature({inputs: _U.list([])
                                                     ,outputs: {onUpdatedMessage: _U.list([A2($Signal.forwardTo,
                                                     aboutFeatureMailbox.address,
                                                     $About$Feature.Update)])}});
   var aboutFeature = $About$Feature.createAboutFeature({inputs: _U.list([aboutFeatureMailbox.signal]),outputs: {}});
   var html = A3($Signal.map2,composeMainView,homeFeature.html,aboutFeature.html);
   var composeRouteMainFeature = {html: html};
   return _elm.ComposeRouteMain.values = {_op: _op
                                         ,aboutFeatureMailbox: aboutFeatureMailbox
                                         ,homeFeature: homeFeature
                                         ,aboutFeature: aboutFeature
                                         ,composeMainView: composeMainView
                                         ,html: html
                                         ,composeRouteMainFeature: composeRouteMainFeature};
};
Elm.Main = Elm.Main || {};
Elm.Main.make = function (_elm) {
   ...
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $ComposeRouteMain = Elm.ComposeRouteMain.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Html = Elm.Html.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var main = $ComposeRouteMain.composeRouteMainFeature.html;
